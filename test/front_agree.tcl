...
# --- and the ledger, which is a FILE both programs write ----------------------
#
# TWO CHECKS, AND THE SECOND ONE IS THE STRONG ONE.
#
# The verdicts must agree: both programs recompute the whole inventory from the
# tree and say which lock files are missing, stale or current. That catches a
# disagreement about CONTENT without either side telling the other what it found.
# Everything but the trailing advice line must be identical -- z says
# `run: z ledger refresh` and machteld says `run: mt ledger refresh`, which is
# the one deliberate difference and is asserted rather than tolerated.
#
# Then, BYTE FOR BYTE, wherever that is provable without writing to the
# workspace. If z does not report a file as stale or missing, the bytes on disk
# ARE z's bytes -- that is precisely what `ok`/silence means -- so comparing
# machteld's generated bytes against the file is a genuine z-vs-machteld byte
# diff with no oracle to build and nothing mutated. When z DOES report a file
# stale the comparison is unavailable, and this says so by name rather than
# passing quietly: a skipped check that looks like a passing one is how step 1
# reported 273/273 while four tools were missing.
proc ledgerLines {text} {
    set out {}
...
set zl [ledgerLines [dict get [run -timeout 300s -- $zexe ledger check] out]]
set ml [ledgerLines [dict get [run -timeout 300s -- [info nameofexecutable] ledger check] out]]
set lbad 0
# The advice line, if any, is the last one and is the one that must differ.
set zadvice "" ; set madvice ""
if {[lindex $zl end] eq "run: z ledger refresh"}  { set zadvice [lindex $zl end] ; set zl [lrange $zl 0 end-1] }
if {[lindex $ml end] eq "run: mt ledger refresh"} { set madvice [lindex $ml end] ; set ml [lrange $ml 0 end-1] }
puts [format "ledger verdicts  : z %d line(s), machteld %d" [llength $zl] [llength $ml]]
if {$zl ne $ml} {
    set lbad 1
    puts "  only z       : [lmap x $zl {expr {$x in $ml ? [continue] : $x}}]"
    puts "  only machteld: [lmap x $ml {expr {$x in $zl ? [continue] : $x}}]"
} else {
    foreach l $zl { puts "                 : $l" }
}
if {($zadvice eq "") != ($madvice eq "")} {
    set lbad 1
    puts "  one side advised a refresh and the other did not: z=\"$zadvice\" mt=\"$madvice\""
}

# The byte comparison, for every lock file z did not call stale or missing.
set lhome [dict get [front roots] home]
set lfiles [::machteld::LedgerFiles]
set lproven 0 ; set lunprovable {}
foreach rel [lsort [dict keys $lfiles]] {
    set complained 0
    foreach l $zl { if {[string match "* $rel" $l]} { set complained 1 } }
    if {$complained} { lappend lunprovable $rel ; continue }
    set path [file join $lhome $rel]
    set got ""
    if {![catch {open $path rb} fh]} { set got [read $fh] ; close $fh }
    if {$got eq [dict get $lfiles $rel]} {
        incr lproven
    } else {
        set lbad 1
        puts "  BYTES DIFFER from z's in $rel ([string length $got] on disk,\
              [string length [dict get $lfiles $rel]] generated)"
    }
}
puts [format "ledger bytes     : %d file(s) byte-identical to z's%s" $lproven \
