...
machteld 0.4.0 is a single-executable Windows runtime for small machine-control
programs. It combines Tcl 9/Tk with native supervision, ConPTY, filesystem
...
4. `worker`, `pool`, `pmap`, `wrap`, `manifest`, and `help` compose and ship them.
...
and data API and version, including static SQLite. Documentation and nested
wrapping remain in the distribution host. It is a deployment form, not a second
language.
