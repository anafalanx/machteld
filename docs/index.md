...
*A single-exe personal Tcl/Tk toolkit for Windows 11: you write Tcl, the power is added in C. The host, the execution core (run / child / wait / scope / detach), pty, store, the workspace front door (`front`, `mt <name>`) and its journal are built — see [the log](log.md).*
...
- [Packaging](packaging.md) — one exe, and what rides inside it: the libraries, the prelude, the docs, the tools.
